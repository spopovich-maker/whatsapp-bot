...
const TWILIO_AUTH_TOKEN = "Y3R4P3SGEZW8AZ3W3F4NG8RP";     // *** CHANGER ***
...
function detectQuickCommand(text) {
    ...
    if (/(bonjour|salut|hello|bonsoir|hey)/.test(t)) return "greeting";
    if (/(prix|tarif|combien|coût)/.test(t)) return "prices";
    if (/(adresse|localisation|où|ou trouver|itinéraire)/.test(t)) return "location";
    if (/(promo|réduction|offre|promotion)/.test(t)) return "promo";
    return null;
}
